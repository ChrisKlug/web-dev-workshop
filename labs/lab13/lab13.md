...
These tests will reside in a new test project. So, go ahead and create a new __xUnit Test Project__ called __WebDevWorkshop.Web.Tests__.
...
Then you can rename __UnitTest1.cs__ to __ShoppingCartTests.cs__, and remove the __Test1__ test.

Inside the `ShoppingCartTests` class, add another public class called __GetShoppingCart__.

__Note:__ This class nesting is only to make the Test Explorer display the tests in a nicer way, and to enable potential sharing of code between the tests.
...
You created this awesome `TestHelper` class in a previous lab. However, that assumes a `DbContext` will be involved. In this case, that is not true. So, you will need to create another `ExecuteTest` method that does not assume a `DbContext`.
...
This new version doesn't need the `TDbContext` type parameter. Because of that, it won't need the `dbSetup` parameter either.
...
This change will obviously cause the code to stop compiling. Luckily, you can just remove everything that isn't working.

So, go ahead and remove everything inside the call to `builder.ConfigureTestServices()`, as well as everything else that has to do with the database setup. You should be left with something that looks like this:
...
Now you have an `ExecuteTest` method that you can use for the __WebDevWorkshop.Web__ project, even if it doesn't utilize a `DbContext`.
...
There are two ways to solve this. Either you spin up an Orleans test cluster, which is the most "correct" way to do it, or you trust Orleans to work and just fake the `IGrainFactory`. For the sake of simplicity, you will do the latter.
...
To fake the `IGrainFactory`, you will need some form of mocking framework. In this lab, you will use one called __FakeItEasy__. So, go ahead and add the NuGet package called __FakeItEasy__.

Then, inside the `serviceConfig`, create a faked `IShoppingCart` instance, and a faked `IGrainFactory` instance to return it based on the grain key __TestCart__.
...
With the cookie in place, you can perform the request as usual, and verify the response.
...
Unfortunately, there is quite a lot of repetition in the tests, even when using the `TestHelper`. In the end, that will be impossible to maintain. So, let's make that a little better by creating a small helper method that removes the repetition for these tests.

At the bottom of the `ShoppingCartTests` class, add a protected, static method called __ExecuteTest__. It should take 2 parameters: a `Func<HttpClient, Task>` called __test__ and an `Action<IGrainFactory, IProductsClient>` called __serviceConfig__. Finally, it needs to return a `Task`.
...
This method will replace the call to `TestHelper.ExecuteTest` in the actual tests, giving the tests a single place to do common setup. In this case, the setup needed is to create the fakes. However, as the fakes need slightly different behavior for each test, the actual behavior config is left up to the `serviceConfig` callback.
...
That is not a huge change, but with a lot of tests, it adds up. It also makes the tests _very_ precise and easy to read and understand.
